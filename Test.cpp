...
using std::cerr; // added FvW
using std::endl; // added FvW
...
float Test::gammq(float a, float x)
{
  ...
  //cout <<"a " << a<< " x "<<x<<endl;
  if(x < 0.0 || a <= 0.0)
    nerror((char*)"Invalid arguments in routine gammq");
...
void Test::gser(float *gamser, float a, float x, float *gln)
{
  ...
  *gln = gammln(a);
  ...
  if(x <= 0.0)
    {
      if(x < 0.0) 
	nerror((char*)"x less than zero in series expansion gamma function");
      *gamser = 0.0;
      return;
    }
  else
      ...
      for(n = 1; n <= ITMAX; n++)
	{
      ...
      nerror((char*)"a is too large, ITMAX is too small, in series expansion gamma function");
      return;
    }
}
...
void Test::gcf(float *gammcf, float a, float x, float *gln)
{
  ...
  *gln = gammln(a);
  ...
  if (i > ITMAX)
    nerror((char*)"a too large, ITMAX too small in continued fraction gamma function");
